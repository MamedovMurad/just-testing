...
const OrganizationAdd: React.FC = () => {
  ...
  const [ministries] = useFetchOrganizations({
    type: "CURATOR",
    related: false,
  });
  ...
  const formik = useFormik({
    initialValues: {
      ...organization,
      relatedMinistry:
        organization.relatedMinistry || createDefaultOrganization(),
    },
    validationSchema,
    ...validationTiming,
  ...
  const ministryOptions = useMemo(() => {
    return ministries.map((ministry) => ({
      id: ministry.id,
      text: `${ministry.name} ${
        ministry.parent?.name ? `(${ministry.parent?.name})` : ""
      }`,
    }));
  }, [ministries]);
  ...
  const relatedMinistryValue = useMemo<SelectValue>(() => {
    return {
      id: values.relatedMinistry?.id || -1,
      text: values?.relatedMinistry?.name || "",
    };
  ...
  const handleRegionChange = useCallback(
    (region: SelectValue) => {
      const selectedRegion =
        regions.find((r) => r.id === region.id) || createDefaultRegion();
  ...
  const handleParentChange = useCallback(
    (parent: SelectValue) => {
      const selectedOrganization =
        structures.find((o) => o.id === parent.id) ||
        createDefaultOrganization();
  ...
  const handleMinistryChange = useCallback(
    (ministry: SelectValue) => {
      const selectedMinistry =
        ministries.find((m) => m.id === ministry.id) ||
        createDefaultOrganization();
  ...
  const typeChangeEffect = () => {
    if (values.type) {
      setIsMinistryRequired(
        values.type.label === "EXECUTIVE" || values.type.label === "OFFICE"
      );
      setIsParentRequired(
        values.type.label === "SUBOFFICE" ||
          values.type.label === "REPRESENTATION"
      );
    }
  };
          ...
          >
            ...
            <div className="col-6 pl-2">
              <div className="pt-6 pb-15 px-6 card">
                ...
                {isSuperAdmin &&
                  isDevelopment &&
                  values.type?.label === "CURATOR" && (
                    <div className="mb-2 d-flex align-center">
                      <Checkbox
                        value={values.askForRedirection || false}
                        onCheck={handleCheck}
                      >
