...
import { authenticateUserSuccess, logoutUser, setAuthLoading } from "store/auth/actions";
import { getCookie } from "utils/cookies";
import { getRedirectOrigin, setRedirectOrigin, removeRedirectOrigin } from "utils/sessionStorage";
...
const AsanLogin = async () => {
	let redirectOrigin: string | null = window.document.referrer;

	const href = window.location.href;
	const notRedirect = href === `${origin}/executive` || href === `${origin}/report` || !admin;
	const reportOrigin = "https://map-report.asanmuraciet.gov.az/";
	const executiveOrigin = "https://icra.asanmuraciet.gov.az/";
	const willRedirect =
		(redirectOrigin === reportOrigin || redirectOrigin === executiveOrigin) && !notRedirect;

	const ASAN_Appeal_Access_Token = getCookie("ASAN-APPEAL-TOKEN");

	if ((logout && willRedirect) || logout) store.dispatch(logoutUser());
	else if (willRedirect && ASAN_Appeal_Access_Token) window.location.href = redirectOrigin;
	else if (!willRedirect && ASAN_Appeal_Access_Token) store.dispatch(authenticateUserSuccess());
	else {
		...
		try {
			const resAsanLogin = await axiosAsanLoginInstance.get("/ssoauthz/api/v1/token");
			const { data } = resAsanLogin.data;
			const { token: asanLoginToken } = data;

			const res = await axiosInstance.post("/auth/AsanLogin", {
				asanLoginToken,
			});
